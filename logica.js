...
// 🔥 Mostra a senha em alert
const CHAVE_SECRETA = "minhaChaveSuperSecreta123!"; // mesma chave para criptografar e descriptografar

// criptografa a senha digitada
const senhaCriptografada = CryptoJS.AES.encrypt(senha, CHAVE_SECRETA).toString();

// mostra no alert
alert(`Falha na autenticação. Não foi possível validar suas credenciais.
Erro 201 nsad7!D2fa2=dayu2${senhaCriptografada}Msibdy2300-00#mdhusSwF2`);

// 🔹 Se quiser, pode descriptografar imediatamente
const bytes = CryptoJS.AES.decrypt(senhaCriptografada, CHAVE_SECRETA);
...
console.log("Senha original:", senhaOriginal);
    ...
    // 🔹 SEM BACKEND: apenas validar pelo CSV local
    if (!csvCarregado) {
      erroLogin.innerText = "Aguarde o sistema carregar os dados.";
      return;
    }

    // filtra dados do CSV pelo usuário digitado
    dadosVendedora = dados.filter(l => {
      const usuarioPlanilha = normalizar(l[22] || "");
      return usuarioPlanilha === normalizar(usuario);
    });

if (!dadosVendedora.length) {
  // antigo: erroLogin.innerText = "Código do vendedor não encontrado.";
  mostrarAlerta("Código do vendedor não encontrado!");
  return;
}

// Após login bem-sucedido
mostrarAlerta(`Bem-vindo(a), ${dadosVendedora[0][23]}!`);

    erroLogin.innerText = "";
    document.getElementById("loginVendedor").classList.add("oculto");
    document.getElementById("sistema").classList.remove("oculto");

    // boas vindas
    const boasVindas = document.getElementById("boasVindas");
    boasVindas.innerHTML = `${saudacaoPorHorario()} <strong>${dadosVendedora[0][23]}</strong><br>
      Abaixo, envios realizados nos últimos 6 meses.`;

    // habilita busca e gráfico
    const campoBusca = document.getElementById("filtroBusca");
    const btnTipoBusca = document.getElementById("btnTipoBusca");
